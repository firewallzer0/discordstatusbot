...
def validateJSON(jsonFile):
    ...
    try:    # Validating whether or not this JSON has the proper fields.
        ...
        print('I: %s -- Validate JSON Thread -- announcementChannel field exists...' % datetime.now())
    except ValueError as err:
    ...
    try:  # Validating whether or not this JSON has the proper fields.
        ...
        value = config['truenasNotifyDB']
        print('I: %s -- Validate JSON Thread -- truenasNotifyDB field exists...' % datetime.now(), value)
    except ValueError as err:
        print('E: %s -- Validate JSON Thread -- JSON invalid - Missing truenasNotifyDB field...' % datetime.now())
        return False, None  # If there is an error JSON is invalid, return False and no data.

    if value == '':
        config['truenasNotifyDB'] = None
